###############################################
#ELYL Table any Admission, alternate censoring#
#Anja Wettstein                               #
# 10/08/2022                                  #
###############################################
...
load(file.path(filepath_est,"elyl_alt_censoring_any_admission.RData"))
estimates <- data.table(LYL_diff_df)
rm(LYL_diff_df)
load(file.path(filepath_boot,"CIs_any_admission_altcensoring.RData"))
CI <- data.table(CIs_any_admission_altcens)
rm(CIs_any_admission_altcens)
...
write_xlsx(x = elyl_all, path=file.path(filepath_write,"eLYL_alt_censoring_any_admission_table.xlsx"), col_names = TRUE)
