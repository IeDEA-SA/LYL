# point estimates for excess life-years lost, any admission type with a single diagnosis
# output: R table "elyl_1diag_any_admission.RData"
...
which_mhd <- c("first_any_admission","first_any_admission_organic","first_any_admission_substance_use_disorder","first_any_admission_alcohol_use_disorder",
...
save_filename <- "elyl_1diag_any_admission.RData"   # setting to NULL prevents saving a data frame with the ELYLs

load(file=paste(filepath_read,"/long_table_Ndiag_any_admission.RData",sep=''))
...
for(ggg in which_sex)
{
  for(v in which_mhd)
  {
    ...
    # excluding patients with two or more diagnoses of the disorder
    N_diag <- gsub("first_","N_",v)
    DT <- DT[get(N_diag)<2]
    
    # age at first mhd treatment of any kind and from any source
    DT[,age_any_treat:=as.numeric(first_any_admission-birth_d)/365.25]
    ...
    print(paste0("Number of deaths in exposed: ",DT_exp[,sum(event)]))
