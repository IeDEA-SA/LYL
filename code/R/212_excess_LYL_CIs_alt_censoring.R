...
# deaths occurring less than a year after patient exit are NOT censored
...
for(v in which_mhd_any)
  for(w in which_sex)
     for(i in 1:10)
     {
       filename <- file.path(filepath_read,paste0("elyl_boot_altcens_",v,"_",w,"_c",i,".RData"))
...
save(eLYLboot_any_admission,file=file.path(filepath_write,"eLYLboot_any_admission_altcensoring.RData"))

# 95% CIs

CIs_any_admission_altcens <- eLYLboot_any_admission[,.(lcl_allcause=quantile(AllCause,0.025),ucl_allcause=quantile(AllCause,0.975),
...
save(CIs_any_admission_altcens,file=file.path(filepath_write,"CIs_any_admission_altcensoring.RData"))
