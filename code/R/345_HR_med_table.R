# produces tables of hazard ratios, medication
...
#read in data
gender <- read.csv(file.path(filepath_lookup,"sex.csv"),header=TRUE, sep=",")
expo <- read.csv(file.path(filepath_lookup,"exposures.csv"),header=TRUE, sep=",")
mod1 <- data.table(read_xlsx(file.path(filepath_read,"aHRs_model1_med.xlsx")))
mod2 <- data.table(read_xlsx(file.path(filepath_read,"aHRs_model2_med.xlsx")))
mod3 <- data.table(read_xlsx(file.path(filepath_read,"aHRs_model3_med.xlsx")))
mod4 <- data.table(read_xlsx(file.path(filepath_read,"aHRs_model4_med.xlsx")))

names(expo)[1] <- "disorder"
gender <- rbind(gender,c("Both","Both"))

target <- c("Any Medication","Antidepressant Medication","Antipsychotic Medication","Anxiolytic Medication","Substance Use Medication")
...
full_table[,`:=`(HRu=gsub("\\.","·",HRu),
                 HRa=gsub("\\.","·",HRa),
                 p_val=gsub("\\.","·",p_val))]
write_xlsx(full_table,file.path(filepath_write,"aHRs_med_table.xlsx"))
