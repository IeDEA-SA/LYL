# Cox Model 2, medication (MED_ATC_N): separate modesl for men and women, no adjustment for psychiatric comorbidities
...
# Run time when analyzing all exposures: ~5 minutes
...
which_mhd<-  c("first_any_med","first_substance_use_med","first_antipsychotic","first_antidepressant","first_anxiolytic")
...
write_xlsx(df_aHRs,path=file.path(filepath_tables,"aHRs_model2_med.xlsx"))
save(creg_list_ac,file=file.path(filepath_write,"obj_model2_ac_med.RData"))
save(creg_list_nd,file=file.path(filepath_write,"obj_model2_nd_med.RData"))
save(creg_list_ud,file=file.path(filepath_write,"obj_model2_ud_med.RData"))
