# Cox Model 4, medication (MED_ATC_N): for men and women and separately, including psychiatric comorbidity
...
which_mhd <- c("first_substance_use_med","first_antipsychotic","first_antidepressant","first_anxiolytic")
...
write_xlsx(df_aHRs,path=file.path(filepath_tables,paste0("aHRs_model4_med.xlsx")))
save(creg_list_ac,file=file.path(filepath_write,"obj_model4_ac_med.RData"))
save(creg_list_nd,file=file.path(filepath_write,"obj_model4_nd_med.RData"))
save(creg_list_ud,file=file.path(filepath_write,"obj_model4_ud_med.RData"))
