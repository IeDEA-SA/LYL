# Forest Plots from Cox Regression, hospitalizations
...
target <- c("Any Mental Health Diagnosis", "Organic Disorders","Substance Use Disorders","Psychotic Disorders",
            "Mood Disorders","Anxiety Disorders","Developmental Disorders","Personality Disorders")
...
mod1_any <- read_xlsx(file.path(filepath_read,"aHRs_model1_hos.xlsx"))
mod2_any <- read_xlsx(file.path(filepath_read,"aHRs_model2_hos.xlsx"))
mod3_any <- read_xlsx(file.path(filepath_read,"aHRs_model3_hos.xlsx"))
mod4_any <- read_xlsx(file.path(filepath_read,"aHRs_model4_hos.xlsx"))
...
mod2_any$ucl <- as.numeric(mod2_any$ucl)  # for some reason the upper limit for this model is a character class
modA <- bind_rows(mod1_any,mod2_any)

##Row bind 3&4
mod3_any$p_int_sex_exp <- NULL
mod3_any$ucl <- as.numeric(mod3_any$ucl)  # for some reason the upper limit for this model is a character class
mod4_any$ucl <- as.numeric(mod4_any$ucl)   
...
p <- ggplot(data=mod_any,aes(x=estimate,y=d_lables))+
  ...
  scale_x_continuous(trans="log2",limits=c(0.4,20),breaks = c(0.5,1,2,4,8),labels=c(0.5,1,2,4,8))+
  xlab("")+
  ylab("")

ggsave(p,filename=file.path(filepath_plot,"aHRs_hos.png"),height=8,width=7,dpi=600)