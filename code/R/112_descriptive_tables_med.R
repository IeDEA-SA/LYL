...
for(v in which_mhd)
{
  ...
  if(v=="first_any_admission")   # creating single table with/without mental illness
  {
    ...
    write_xlsx(overall_df,path=file.path(filepath_write,"descriptive_overall_med.xlsx"))
  }
...
MHD_count_df[,`:=`(men=gsub("\\.","·",men),
                   women=gsub("\\.","·",women),
                   total=gsub("\\.","·",total))]
write_xlsx(MHD_count_df,path=file.path(filepath_write,"descriptive_by_MHD_and_sex_med.xlsx"))
