#############################################
#ELYL Table any Admission, single diagnoses #
#Anja Wettstein                             #
# 10/08/2022                                #
#############################################
...
load(file.path(filepath_est,"elyl_1diag_any_admission.RData"))
estimates <- data.table(LYL_diff_df)
rm(LYL_diff_df)
load(file.path(filepath_boot,"CIs_any_admission_1diag.RData"))
CI <- data.table(CIs_any_admission_2diag)
rm(CIs_any_admission_2diag)
...
write_xlsx(x = elyl_all, path=file.path(filepath_write,"eLYL_1diag_any_admission_table.xlsx"), col_names = TRUE)
