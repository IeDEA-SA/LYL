# Cox Model 3, hospital admissions (HOS): including psychiatric comorbidity, adjusting for sex
...
for(v in which_mhd)
{
  ...
  print("Model:")
  cformula <- as.formula(paste0("Surv(start,stop,event) ~ strata(sex) + strata(age_enrol_cat) + strata(calyear_cat) + ",paste0("exp_",v," + "),paste0(comorb_vect,collapse=" + ")))
  print(cformula)
  print("*****")
  ...
  df_aHRs <- rbind(df_aHRs,
                   data.frame(disorder=v,sex="Both",cause="all",estimate=X_ac[1],
                              lcl=X_ac[3],ucl=X_ac[4],
                              p_ph_exp=zph_ac[3]),
                   data.frame(disorder=v,sex="Both",cause="Natural",estimate=X_nd[1],
                              lcl=X_nd[3],ucl=X_nd[4],
                              p_ph_exp=zph_nd[3]),
                   data.frame(disorder=v,sex="Both",cause="Unnatural",estimate=X_ud[1],
...
write_xlsx(df_aHRs,path=file.path(filepath_tables,paste0("aHRs_model3_hos.xlsx")))
save(creg_list_ac,file=file.path(filepath_write,"obj_model3_ac_hos.RData"))
save(creg_list_nd,file=file.path(filepath_write,"obj_model3_nd_hos.RData"))
save(creg_list_ud,file=file.path(filepath_write,"obj_model3_ud_hos.RData"))
