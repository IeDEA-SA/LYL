# Forest Plots from Cox Regression, any admission type
...
mod1_any <- read_xlsx(file.path(filepath_read,"aHRs_model1_any_admission.xlsx"))
mod2_any <- read_xlsx(file.path(filepath_read,"aHRs_model2_any_admission.xlsx"))
mod3_any <- read_xlsx(file.path(filepath_read,"aHRs_model3_any_admission.xlsx"))
mod4_any <- read_xlsx(file.path(filepath_read,"aHRs_model4_any_admission.xlsx"))
...
p <- ggplot(data=mod_any,aes(x=estimate,y=d_lables))+
  ...
  scale_x_continuous(trans="log2",limits=c(0.5,8),breaks = c(0.5,1,2,4,8),labels=c(0.5,1,2,4,8))+
  xlab("")+
  ylab("")

ggsave(p,filename=file.path(filepath_plot,"aHRs_any_admission.png"),height=8,width=7,dpi=600)