...
load(file.path(filepath_est,"elyl_med.RData"))
estimates <- data.table(LYL_diff_df)
rm(LYL_diff_df)
load(file.path(filepath_boot,"CIs_med.RData"))
CI <- data.table(CIs_med)
rm(CIs_med)
...
write_xlsx(x = elyl_all, path=file.path(filepath_write,"eLYL_med_table.xlsx"), col_names = TRUE)
